    ...
    private fun drawEditor(mouseX: Int, mouseY: Int) {
        ...
        if (Mouse.isButtonDown(0) && !mouseDown && mouseX >= x && mouseX <= x + width && mouseY >= y + height &&
            ...
            element.x = when (element.side.horizontal) {
                ...
                Side.Horizontal.MIDDLE -> (ScaledResolution(mc2).scaledWidth / 2) - x
                Side.Horizontal.RIGHT -> ScaledResolution(mc2).scaledWidth - x
            }
        }
        ...
        if (Mouse.isButtonDown(0) && !mouseDown && mouseX >= x && mouseX <= x + width && mouseY >= y + height && mouseY <= y + height + 10) {
            ...
            element.y = when (element.side.vertical) {
                Side.Vertical.UP -> y
                Side.Vertical.MIDDLE -> (ScaledResolution(mc2).scaledHeight / 2) - y
                Side.Vertical.DOWN -> ScaledResolution(mc2).scaledHeight - y
            }
        }
            ...
            when (value) {
                ...
                is FontValue -> {
                    ...
                    val text = when (fontRenderer) {
                        ...
                        mc.fontRendererObj -> "Font: Minecraft"
                        else -> "Font: Unknown"
                    }

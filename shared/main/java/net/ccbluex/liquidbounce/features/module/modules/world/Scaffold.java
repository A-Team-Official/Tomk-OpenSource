...
public class Scaffold extends Module {
    ...
    private final float antiX = 0F;
    private final float antiY = 0F;
    ...
    public void shader(final BlurEvent event) {
        ...
        if (blockCounter.get() == "Rise") {
            float realY = height + 10;
            float nowTime = System.currentTimeMillis();
            //Y-Axis Animation
            if (nowY != realY) {
                float pct = (nowTime - animeYTime) / 500;
                if (pct > 1) {
                    nowY = realY;
                    pct = 1.0F;
                } else {
                    pct = (float) EaseUtils.INSTANCE.easeOutBack(pct);
                }
                GL11.glTranslated(0.0, (realY - nowY) * pct, 0.0);
            } else {
                animeYTime = nowTime;
            }
            GL11.glTranslated(0.0, nowY, 0.0);
            progress = (float) (System.currentTimeMillis() - lastMS) / 600F;
            if (progress >= 1) progress = 1;
           progress = (float) EaseUtils.INSTANCE.easeOutBack(progress);
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        int infoWidth = Fonts.sfbold28.getStringWidth(info);
        ...
        if (blockCounter.get() == ("MC")) {
            ...
            if (itemStack != null) {
                ...
                Fonts.sfbold28.drawCenteredString("?", sr.getScaledWidth() / 2F + 0.5F, height + 6, -1);
        }

        if (blockCounter.get() == ("Rise")) {
            GlStateManager.resetColor();
            final int height = sr.getScaledHeight() - 120;
            ShadowUtils.INSTANCE.shadow(8f, () -> {
                GL11.glPushMatrix();
...
//            RenderUtils.drawShadow((sr.getScaledWidth() / 2F) - 15, height, 30, 35);
            if (itemStack != null) {
                Fonts.sfbold40.drawCenteredString(String.valueOf("Amount: " + ChatFormatting.YELLOW +getBlocksAmount()), sr.getScaledWidth() / 2F, height -2, -1);
                GlStateManager.pushMatrix();
                GlStateManager.enableRescaleNormal();
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                RenderHelper.enableGUIStandardItemLighting();
                mc2.getRenderItem().renderItemAndEffectIntoGUI(itemStack, (int) (sr.getScaledWidth() / 2F - 44.5), height - 6);
                GlStateManager.disableRescaleNormal();
                GlStateManager.disableBlend();
                RenderHelper.disableStandardItemLighting();
                GlStateManager.popMatrix();
            } else {
                Fonts.sfbold35.drawCenteredString("?", sr.getScaledWidth() / 2F + 0.5F, height - 54, -1,true);
            }
            GlStateManager.resetColor();
            GL11.glPopMatrix();
        }
        if (blockCounter.get() == ("Novoline")) {
            ...
            if (itemStack != null) {
                Fonts.sfbold35.drawCenteredString(String.valueOf(getBlocksAmount()), sr.getScaledWidth() / 2F, height + 40 - 17, -1);
                GlStateManager.pushMatrix();
                GlStateManager.enableRescaleNormal();
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                ...
                Fonts.sfbold35.drawCenteredString("?", sr.getScaledWidth() / 2F + 0.5F, height + 6, -1);
            }
            GlStateManager.resetColor();
            GL11.glPopMatrix();
        }
